class Account:
    def __init__(self, name,acc_number):
        ...
        self.withdrawals = []
        self.loan = 0
        # self.time = datetime.now().strftime("%x %X")
    ...
    def withdraw(self,amount):
        if amount + self.transaction > self.balance:
            return "Your balance is {self.balance} you can not withdraw {amount}"
        elif amount <= 0:
            return f"Amount must be greater than zero"
        else:
            self.balance -= amount + self.transaction
            withdrawal_dictionary = {
                "date": datetime.now().strftime("%x %X"),
                "amount": amount,
                "narration": f"You have withdrawn {amount} your balance is {self.balance}"
            }
            self.withdrawals.append(withdrawal_dictionary)
            return f"You have withdrawn {amount} your balance is {self.balance}"
    def deposits_statement(self):
        for statements in self.deposits:
            print(statements["narration"])
    def withdraws_statement(self):
        for staments in self.withdraws:
            print(staments["narration"])
    ...
    def full_statements(self):
        ...
        for statement in full_statements_list:
            if statement in self.withdrawals:
    ...
    def loan_repayment(self,amount):
        if amount<=0:
            return "Dear customer, You can not pay 0 amount"
        if amount>self.loan:
